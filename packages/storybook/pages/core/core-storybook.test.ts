...
describe("@nodes/core centralized storybook", () => {
  ...
  test("keeps the live detail visible on package overview and exact leaf", async () => {
    const entry = await Bun.file(join(storybookRoot, "live-node-tree.stories.ts")).text()
    const detail = await Bun.file(join(storybookRoot, "core-detail.ts")).text()
    const body = await Bun.file(join(storybookRoot, "core-storybook-body.html")).text()
