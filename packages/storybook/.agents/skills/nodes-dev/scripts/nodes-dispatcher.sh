...
port=$registry_port
test_override=false
if [[ -n ${NODES_DEV_TEST_PORT:-} ]]; then
  [[ ${NODES_DEV_TEST_MODE:-0} == 1 ]] \
    || die "NODES_DEV_TEST_PORT requires NODES_DEV_TEST_MODE=1"
  [[ ${NODES_DEV_TEST_PORT} =~ ^[0-9]+$ ]] \
    && ((NODES_DEV_TEST_PORT > 0 && NODES_DEV_TEST_PORT < 65536)) \
    || die "NODES_DEV_TEST_PORT must be an integer from 1 to 65535"
  port=$NODES_DEV_TEST_PORT
  test_override=true
fi
...
temp_root=${temp_root%/}
state_root=${NODES_DEV_STATE_ROOT:-$temp_root/nodes-dev}
...
status_json() {
  ...
  if [[ $status == stopped && $last_exit_reason == owner-session-lost ]]; then
    recovery_hint="Run nodes-dev.sh ensure in a long-lived PTY before the next lifecycle or browser operation."
  fi
