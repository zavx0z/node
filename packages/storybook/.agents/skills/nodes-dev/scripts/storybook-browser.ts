...
if (action === undefined || checkoutInput === undefined || selectorOrUrl === undefined) {
  fail("usage: storybook-browser.ts {targets|target|open|close|reload|dom|console|page|canvas|viewports|touch|profile|interact} <checkout> <selector|exact-url> [options]")
}
...
const cdpPort = Number(Bun.env.NODES_DEV_CDP_PORT ?? 9222)
if (!Number.isInteger(cdpPort) || cdpPort < 1 || cdpPort > 65535) fail("NODES_DEV_CDP_PORT must be 1..65535")
...
function resolveTargetConfig(registry: Registry, checkout: string, input: string, options: Options): TargetConfig {
  ...
  if (Bun.env.NODES_DEV_TEST_PORT) {
    if (Bun.env.NODES_DEV_TEST_MODE !== "1") fail("NODES_DEV_TEST_PORT requires NODES_DEV_TEST_MODE=1")
    port = positiveInteger(Bun.env.NODES_DEV_TEST_PORT, "NODES_DEV_TEST_PORT")
    if (port > 65535) fail("NODES_DEV_TEST_PORT must be <= 65535")
    testOverride = true
  }
